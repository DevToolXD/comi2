...
## 두 가지 버전 (Two editions)

| | **① 터미널 버전** (`scan.py`) | **② 브라우저 F12 콘솔 버전** (`browser_console_scanner.js`) |
|---|---|---|
| 실행 위치 | 명령 프롬프트/터미널 (Python) | 브라우저 개발자도구(F12) → Console 탭에 붙여넣기 |
| 범위 | 서버 전체 (외부에서 블랙박스) | 지금 열려 있는 **내 사이트(같은 출처)** |
| 강점 | **포트 스캔, TLS 인증서, 서버 노출** + 전 항목 | 설치 없이 즉시, 로그인 세션 그대로 점검 |
| 한계 | 로그인 뒤 영역은 쿠키 지정 필요 | 브라우저 정책상 **포트/TLS/타 도메인/HttpOnly 쿠키 불가** |

두 버전은 상호보완입니다. 포트·TLS·서버 노출까지 보려면 ①, 설치 없이 F12에서
바로 돌리려면 ②를 쓰세요. **둘 다 SQLi·XSS·경로탐색·명령주입·민감파일·헤더·
CSRF 를 실제로 점검**합니다.

### ② 브라우저 F12 콘솔 버전 — 설치 없이 바로

1. 점검할 **내 사이트**를 브라우저에서 엽니다.
2. `F12` → **Console** 탭.
3. `browser_console_scanner.js` **파일 전체**를 복사해 붙여넣고 Enter.
   (Chrome이 붙여넣기를 막으면 콘솔에 `allow pasting` 을 한 번 입력 후 다시 붙여넣기.)

```js
// 붙여넣으면 자동 실행됩니다. 다시 돌리거나 옵션을 주려면:
SITESCAN();                    // 기본: 같은 출처 능동 점검(비파괴 GET) + 수동
SITESCAN({ deep:false });      // 수동만 — 페이로드 전송 안 함(가장 안전)
SITESCAN({ includePost:true });// POST 폼까지 능동 점검(부작용 가능, 주의)
copy(JSON.stringify(window.__scan, null, 2));  // 전체 결과 JSON 복사
```

결과는 콘솔에 심각도별 색상 로그 + 표로 뜨고 `window.__scan` 에 저장됩니다.

---

## 빠른 시작 — ① 터미널 버전 (Quick start)
